...
<Nonce><![CDATA[%(nonce)s]]></Nonce>
    ...
    def extractByKey(self,xmltext,key):
        """提取出xml数据包中的加密消息
                @param xmltext: 待提取的xml字符串
                @return: 提取出的加密消息字符串
                """
        try:
            xml_tree = ET.fromstring(xmltext)
            keyData = xml_tree.find(key)
            return ierror.WXBizMsgCrypt_OK, keyData.text
        except Exception, e:
            # print e
...
if __name__ == '__main__':
    ...
    data = '<xml><ToUserName><![CDATA[wwbab257c6769edaa3]]></ToUserName><CreateTime>1657879381</CreateTime><MsgType><![CDATA[event]]></MsgType><Event><![CDATA[kf_msg_or_event]]></Event><Token><![CDATA[ENC7qFtA2HcjxRrMi3tG7B3VbLpbXA1sgsu7U5YKUwiVNE2]]></Token></xml>'
    _XMLParse = XMLParse()
    ret,text = _XMLParse.extractByKey(data,"Token")
    print ret,text